...
const rowdies1 = Rowdies({ weight: "700", display: "swap", subsets: ["latin"] })
const shadows1 = Shadows_Into_Light({ weight: "400", display: "swap", subsets: ["latin"] })
...
const Verify = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post("/api/auth/verify-otp", { email, otp: enteredOtp })
      if (res.data.success) {
  ...
  return (
    <div className="w-screen h-screen flex">
      ...
      <div className="text-white min-h-screen w-[500px] bg-black flex flex-col justify-center gap-9 items-center px-5 pt-12">
        <h2 className={`${rowdies1.className} text-6xl text-white font-bold`}>OTP SENT</h2>
        <p className={`${rowdies1.className} text-xl`}>OTP sent to your Registered Email address</p>
        <form onSubmit={handleSubmit} className="flex flex-col items-center gap-7 w-full px-4">
          ...
          <button className="w-full bg-[#0c1feb] h-[45px] flex justify-center items-center rounded-md text-xl">
            <p className={rowdies1.className}>{isSubmitting ? "Verifying" : "Verify OTP"}</p>
          </button>
        </form>
      </div>
    </div>
